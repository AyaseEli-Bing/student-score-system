...
#define NAME_LEN     20   /* 姓名缓冲区的长度：所有数组都写 NAME_LEN，末尾留 1 位放 '\0' */

/* 结构体：把"一个学生"的多条数据打包成一个整体 */
struct Student {
    char name[NAME_LEN];   /* 姓名：最多 NAME_LEN-1 个字符，末尾要留 1 个位置放结束标记 '\0' */
...
int main(void) {
    char line[NAME_LEN];   /* 存放用户敲的一整行内容 */
    int  choice;           /* 用户在菜单里输入的操作编号 */
    int  state;            /* read_line 的返回状态 */
    ...
    while (1) {                /* 一直循环，直到用户选择"退出" */
        ...
        /* 不处理它的话，getchar 会永远返回 EOF，程序就会疯狂重复提示。 */
        if (state == -1) {
            printf("\n输入已结束，程序退出。\n");
            return 0;
        }
...
int line_to_number(char line[]) {
    ...
    value = 0;
    for (i = 0; line[i] != '\0'; i++) {
        digit = line[i] - '0';   /* 字符 '0' 到 '9' 的编码是连续的，减一下就是数值 */
        if (digit < 0 || digit > 9) {
            return -1;
        }
...
/* 公共代码：读一段必须有内容的文本（用来读姓名和确认词） */
/* 只按回车不输入，就反复要求重读——不会把用户的下一次输入吞掉 */
/* 返回值：1 = 拿到了内容；0 = 输入已结束 */
int read_text(char text[]) {
    int state;

    while (1) {
        state = read_line(text);
        if (state == -1) {
            printf("! 输入已结束，已返回主菜单。\n");
            return 0;
        }
...
int read_valid_score(char name[]) {
    ...
    while (1) {
        ...
        return score;   /* 到这里说明成绩合法（能走到这里，score 必然 >= 0） */
    }
}
...
/* 功能 1：录入学生成绩 */
void input_students(void) {
    char temp_name[NAME_LEN];   /* 临时存放姓名，确认数据都合法后再写进数组 */
    int  score;
    ...
    while (student_count < MAX_STUDENTS) {   /* 表格没满才继续录入 */
        printf("\n请输入学生姓名（输入 0 结束录入）：");
        if (read_text(temp_name) == 0) {
            return;   /* 输入已结束：直接离开函数，回到主菜单 */
        }
        ...
        /* 重名会让查询、修改、删除永远只认第一条记录，所以录入时就拒绝 */
        if (find_index_by_name(temp_name) != -1) {
            printf("! 已经录入过 %s，请换一个姓名；要改成绩请用功能 5。\n", temp_name);
            continue;
        }
...
void show_statistics(void) {
    ...
    pass_count = student_count - seg_fail;   /* 60 分及以上都算及格 */
    ...
    /* 人数做除法会得到 0，所以要先把一个数转成 double；%% 是打印一个真正的百分号 */
    printf("及格率：%.1f%%（%d/%d 人，60 分及以上为及格）\n",
           (double)pass_count / student_count * 100, pass_count, student_count);
...
void show_ranking(void) {
    ...
    int i, j;
    int rank = 1;                          /* 给个初值，别留下"可能没赋值就用"的隐患 */
